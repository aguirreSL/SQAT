...
%   its parameters with their default values, and the time series that the
%   interface plots.
%
% OUTPUT
...
%       * series       - {time field, value field} of the time series in OUT
%       * series_label - label of the time series, with its unit
%
...
metrics = struct('id', {}, 'label', {}, 'params', {}, 'run', {}, 'series', {}, 'series_label', {});
...
metrics(end+1) = il_entry('Loudness_ISO532_1', 'Loudness (ISO 532-1)', ...
    ...
    {'time', 'InstantaneousLoudness'}, 'Loudness (sone)');
...
metrics(end+1) = il_entry('Loudness_ECMA418_2', 'Loudness (ECMA-418-2)', ...
    ...
    {'timeOut', 'loudnessTDep'}, 'Loudness (sone_{HMS})');
...
metrics(end+1) = il_entry('Sharpness_DIN45692', 'Sharpness (DIN 45692)', ...
    ...
    {'time', 'InstantaneousSharpness'}, 'Sharpness (acum)');
...
metrics(end+1) = il_entry('Roughness_Daniel1997', 'Roughness (Daniel 1997)', ...
    ...
    {'time', 'InstantaneousRoughness'}, 'Roughness (asper)');
...
metrics(end+1) = il_entry('Roughness_ECMA418_2', 'Roughness (ECMA-418-2)', ...
    ...
    {'timeOut', 'roughnessTDep'}, 'Roughness (asper_{HMS})');
...
metrics(end+1) = il_entry('FluctuationStrength_Osses2016', 'Fluctuation strength (Osses 2016)', ...
    ...
    {'time', 'InstantaneousFluctuationStrength'}, 'Fluctuation strength (vacil)');
...
metrics(end+1) = il_entry('Tonality_Aures1985', 'Tonality (Aures 1985)', ...
    ...
    {'time', 'InstantaneousTonality'}, 'Tonality (t.u.)');
...
metrics(end+1) = il_entry('Tonality_ECMA418_2', 'Tonality (ECMA-418-2)', ...
    ...
    {'timeOut', 'tonalityTDep'}, 'Tonality (tu_{HMS})');
...
metrics(end+1) = il_entry('PsychoacousticAnnoyance_Widmann1992', 'Annoyance (Widmann 1992)', pa_params, ...
    ...
    {'time', 'InstantaneousPA'}, 'Psychoacoustic annoyance');
...
metrics(end+1) = il_entry('EPNL_FAR_Part36', 'EPNL (FAR Part 36)', ...
    ...
    {'time', 'PNLT'}, 'PNLT (TPNdB)');

end

function e = il_entry(id, label, params, run, series, series_label)
e = struct('id', id, 'label', label, 'params', params, 'run', run, ...
           'series', {series}, 'series_label', series_label);
end
